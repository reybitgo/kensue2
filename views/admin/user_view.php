    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-12 col-md-6">
        <div class="card h-100">
          ...
          <div class="card-body">
            <table class="info-table">
              <tr><td>Package</td><td><span class="badge bg-primary-subtle text-primary"><?= e($user['package_name']??'—') ?></span></td></tr>
              <tr><td>Sponsor</td><td><?= ($user['sponsor_username']??null) ? '<a href="'.APP_URL.'/?page=admin_user_view&id='.$user['sponsor_id'].'">@'.e($user['sponsor_username']).'</a>' : '—' ?></td></tr>
              <tr><td>Upline</td><td><?= ($user['binary_parent_username']??null) ? '@'.e($user['binary_parent_username']).' ('.$user['binary_position'].')' : '—' ?></td></tr>
              <tr><td>Pairing Bonus</td><td><?= fmt_money($user['pairing_bonus']??0) ?> / pair</td></tr>
              <tr><td>Daily Cap</td><td><?= $user['daily_pair_cap']??0 ?> pairs / day</td></tr>
            </table>
            <div class="row g-2 mt-2">
              <div class="col-6"><div class="leg-box text-center"><div class="leg-label">↙ Left</div><div class="leg-count"><?= number_format($user['left_count']) ?></div></div></div>
              <div class="col-6"><div class="leg-box text-center"><div class="leg-label">↘ Right</div><div class="leg-count"><?= number_format($user['right_count']) ?></div></div></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div class="table-responsive">
        <table class="table table-hover mb-0">
          ...
          <tbody>
          <?php if (empty($commHist['data'])): ?><tr><td colspan="6" class="text-center py-4 text-muted">No commissions.</td></tr>
          <?php else: foreach ($commHist['data'] as $c):
            $tn = match($c['type']){'pairing'=>'🤝 Pairing','direct_referral'=>'👥 Direct','indirect_referral'=>'🔗 Indirect Lvl '.$c['level'],default=>$c['type']};
          ?>
          <tr>
      ...
      <div class="table-responsive">
        <table class="table table-hover mb-0">
          ...
          <tbody>
          <?php if (empty($ledger['data'])): ?><tr><td colspan="5" class="text-center py-4 text-muted">No ledger entries.</td></tr>
      ...
      <div class="table-responsive">
        <table class="table table-hover mb-0">
          ...
          <tbody>
          <?php if (empty($payouts['data'])): ?><tr><td colspan="6" class="text-center py-4 text-muted">No payout history.</td></tr>
          ...
          <tr>
            ...
            <td><?php $b=match($pr['status']){'pending'=>'bg-warning-subtle text-warning','approved'=>'bg-info-subtle text-info','completed'=>'bg-success-subtle text-success','rejected'=>'bg-danger-subtle text-danger',default=>'bg-secondary-subtle'}; ?><span class="badge <?= $b ?>"><?= ucfirst($pr['status']) ?></span></td>
