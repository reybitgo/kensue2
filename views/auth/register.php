  ...
  <div class="auth-card auth-card-wide">
    ...
    <form method="POST" action="<?= APP_URL ?>/?page=do_register" id="regForm">
      ...
      <div class="auth-body" id="step1">
        <p class="text-muted mb-3" style="font-size:.85rem;">Enter the registration code given by your sponsor.</p>
        <div class="mb-3">
        ...
        <div id="packageInfo" class="code-verified d-none">
          ...
          <div><div class="fw-bold" id="pkgName"></div><div style="font-size:.75rem;margin-top:2px;" id="pkgDetails"></div></div>
        </div>
      ...
      <div class="auth-body" id="step2" style="display:none;">
        <div class="mb-3">
          ...
          <input type="text" id="username" name="username" class="form-control" placeholder="3–40 chars, letters/numbers/_" minlength="3" maxlength="40" autocomplete="off" required>
        ...
        <div class="mb-3">
          ...
          <input type="text" id="sponsor_username" name="sponsor_username" class="form-control" placeholder="Your sponsor's username" autocomplete="off" required>
          <div class="form-text" id="sponsorHint"></div>
        </div>
        <div class="mb-3">
          <label class="form-label">Binary Upline Username <span class="text-danger">*</span></label>
          <input type="text" id="upline_username" name="upline_username" class="form-control" placeholder="Your upline in the binary tree" autocomplete="off" required>
      ...
      <div class="auth-body" id="step3" style="display:none;">
        <p class="text-muted mb-3" style="font-size:.85rem;">Please review your information before completing registration.</p>
        <div class="card mb-3">
          ...
          <div class="card-body">
            <table class="info-table">
              ...
              <tr><td>Binary Upline</td><td id="rev_upline">—</td></tr>
        ...
        <div class="d-flex gap-2">
          ...
          <button type="submit" class="btn btn-primary flex-grow-1 btn-lg" id="submitBtn">✓ Complete Registration</button>
        </div>
      </div>
    </form>

    <div class="auth-footer">Already have an account? <a href="<?= APP_URL ?>/?page=login">Sign in →</a></div>
  </div>
</div>
...
document.getElementById('validateCodeBtn').addEventListener('click', async function() {
  ...
  try {
    ...
    if (data.valid) {
      ...
      document.getElementById('pkgDetails').textContent = 'Entry: '+data.entry_fee+' · Bonus: '+data.pairing_bonus+' · Cap: '+data.daily_cap+' pairs/day';
    ...
    } else { setHint('codeHint', data.message||'Invalid code.',false); }
  } catch(e) { setHint('codeHint','Network error.',false); }
  this.disabled=false; this.textContent='Validate';
});

document.getElementById('toStep2Btn').addEventListener('click', () => { if (document.getElementById('validatedCode').value) goStep(2); });
...
document.getElementById('sponsor_username').addEventListener('input',function(){
  ...
  const v=this.value.trim(); if(!v){setHint('sponsorHint','',null);return;}
...
document.getElementById('upline_username').addEventListener('input',function(){
  ...
  const v=this.value.trim();
  if(!v){setHint('uplineHint','',null);document.getElementById('slotStatus').classList.add('d-none');resetPosBtns();return;}
  setHint('uplineHint','Checking…',null);
  upTimer=setTimeout(()=>checkUpline(v),600);
});

document.querySelectorAll('[name=binary_position]').forEach(r=>r.addEventListener('change',function(){checkPos(this.value);}));
...
async function checkUpline(v) {
  ...
  if(!data.valid){setHint('uplineHint','✗ '+data.message,false);document.getElementById('slotStatus').classList.add('d-none');uplineOk=false;return;}
  slotData=data; uplineOk=true;
  setHint('uplineHint','✓ Found @'+data.username,true);
  const ls=document.getElementById('leftSlot'), rs=document.getElementById('rightSlot');
  ls.textContent='↙ Left: '+(data.left_free?'✓ Free':'✗ Taken'); ls.className=data.left_free?'slot-free':'slot-taken';
  rs.textContent='↘ Right: '+(data.right_free?'✓ Free':'✗ Taken'); rs.className=data.right_free?'slot-free':'slot-taken';
  ...
  else if(cur==='right'&&!data.right_free&&data.left_free) document.getElementById('pos_left').checked=true;
...
function checkPos(pos){
  ...
  setHint('positionHint',(free?'✓ ':'✗ ')+pos.charAt(0).toUpperCase()+pos.slice(1)+' slot is '+(free?'available.':'taken.'),free);
}
...
document.getElementById('toStep3Btn').addEventListener('click',function(){
  const pw=document.getElementById('password').value, pwc=document.getElementById('password_confirm').value;
  const pos=document.querySelector('[name=binary_position]:checked')?.value;
  ...
  if(!(pos==='left'?slotData.left_free:slotData.right_free)){setHint('positionHint','Selected position is taken.',false);return;}
  ...
  document.getElementById('rev_sponsor').textContent  = '@'+document.getElementById('sponsor_username').value;
